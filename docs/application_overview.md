...
本アプリケーションは PWA 機能を備えており、以下の利点を提供します：
*   **オフライン動作**: キャッシュされたアセットにより、ネットワークが不安定な環境でも起動可能です。
*   **インストール可能**: スマホや PC のホーム画面に追加し、ネイティブアプリのように利用できます。
*   **スタンドアロン表示**: ブラウザの UI（アドレスバー等）を隠し、フルスクリーンで没入感のある体験を提供します。
...
また、GitHub の `main` ブランチにプッシュすると、GitHub Actions が自動的にビルドと Firebase Hosting へデプロイを実行します。
